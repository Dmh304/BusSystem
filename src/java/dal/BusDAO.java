...
public class BusDAO extends DBContext {
    ...
    public List<Bus> getUnassignedBuses() {
        List<Bus> list = new ArrayList<>();
        String sql = "SELECT b.BusID, b.PlateNumber, b.BusName, b.Capacity, b.[Status], "
                + "NULL AS RouteName, NULL AS ManagerName, NULL AS DriverName "
                + "FROM Bus b "
                + "WHERE b.[Status] = 'ACTIVE' AND b.BusID NOT IN ("
                + "SELECT BusID FROM BusAssignment WHERE [Status] = 'ACTIVE')";
        try (PreparedStatement ps = connection.prepareStatement(sql);
             ResultSet rs = ps.executeQuery()) {
            while (rs.next()) {
                list.add(extractBus(rs));
    ...
    public boolean createBusAssignment(int busId, int routeId, int managerUserId, int driverUserId) {
    ...
    public boolean hasAssignment(int managerUserId) {
